...
public abstract class Command implements Sendable {
  ...
  /**
   * Decorates this Command so that its `Initialize`, `Execute`, `isFinished` and `End` methods are
   * implicitly timed using the {@link Tracer} class This decorator also names the command similarly
   * to the {@link #withName(String)} method.
   *
   * @return the decorated Command
   */
  public WrapperCommand traced() {
    return traced(getName());
  }
