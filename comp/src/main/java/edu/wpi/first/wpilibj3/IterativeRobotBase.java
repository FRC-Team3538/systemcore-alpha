...
package edu.wpi.first.wpilibj3;
...
public abstract class IterativeRobotBase extends RobotBase {
  ...
  protected void loopFunc() {
    ...
    Tracer.traceFunc("DriverStation.refreshData()", DriverStation::refreshData);
    m_watchdog.reset();

    Tracer.traceFunc("DSControlWord.refresh()", m_word::refresh);
    ...
    if (!m_calledDsConnected
        && Tracer.traceFunc("DsControlWord.isDSAttached()", m_word::isDSAttached)) {
      m_calledDsConnected = true;
      Tracer.traceFunc("driverStationConnected", this::driverStationConnected);
    }
    ...
    // Flush NetworkTables
    if (m_ntFlushEnabled) {
      Tracer.traceFunc("NetworkTables.flushLocal()", NetworkTableInstance.getDefault()::flushLocal);
    }
