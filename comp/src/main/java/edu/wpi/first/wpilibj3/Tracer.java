...
public class Tracer {
  ...
  private static final class TracerState {
    ...
    private TracerState(String name, boolean threadLocalConstruction) {
      ...
      var gcTopic = m_rootTable.getDoubleTopic("GCTime");
      this.m_gcTimeEntry = gcTopic.publish();
    }
    ...
    private void endCycle() {
      if (m_disabled != m_disableNextCycle || m_cyclePoisoned) {
        ...
        for (var traceTime : m_traceTimes.entrySet()) {
          var traceTopic = m_rootTable.getDoubleTopic(traceTime.getKey());
          DoublePublisher publisher = traceTopic.publish();
